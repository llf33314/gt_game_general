  ...
  created() {
    api.getActivityById({ id: this.$route.query.id }).then(res => {
      if (res.code == 100) {
        ...
        this.ruleForm1.advertisingPictureList = res.data.advertisingPictureList;
        //规则设置
        this.ruleForm2.followQrCode = res.data.followQrCode;
        ...
        this.ruleForm2.actRule = res.data.actRule;
        //兑奖设置
        this.ruleForm3.prizeDate = [
          res.data.cashPrizeBeginTime,
          res.data.cashPrizeEndTime
        ];
        this.ruleForm3.receiveTypeList = res.data.receiveTypeList;
        this.ruleForm3.phone = res.data.phone;
        this.ruleForm3.cashPrizeInstruction = res.data.cashPrizeInstruction;

        //兑奖地址
        var newaddr = [];
        for (var i = 0; i < res.data.addressList.length; i++) {
          var newabc1 = {
            list: res.data.addressList[i]
          };
          newaddr.push(newabc1);
        }
        this.ruleForm3.addressList = newaddr;
        //奖项设置
        this.prizeDescription = res.data.prizeDescription

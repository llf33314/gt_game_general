...
import { 
  ...
  export default{
    ...
    methods: { 
      ...
      //核销授权
      impower(val){
         this.$router.push({path: '/standStill/cancelOut', query: {id: val}});
      },
      userList(val){
         this.$router.push({path: '/standStill/user', query: {id: val}}); 
      },
      addActive(){
        this.$router.push('/standStill/addAct')
      },
      editActive(val){
        this.$router.push({path:'/standStill/editAct', query: {id: val}});
      },
      test(){
        console.log(123)
      }

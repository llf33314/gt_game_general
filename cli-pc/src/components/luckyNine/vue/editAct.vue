...
export default {
  ...
  methods: { 
    ...
    addForm4(){ 
        this.ruleForm4.push({ type: "", prizeUnit: "", prizeName: "", num: "", probabiliy: '', imgUrl : [] },)
    },
    delForm4(val){
      ...
      api.getActivityById({ id: this.$route.query.id }).then(res => {
        if (res.code == 100) {
            ...
            for(let key in this.ruleForm1) {
              this.ruleForm1[key] = res.data[key] || ''
            }
            this.ruleForm1.date = [ res.data.activityBeginTime,  res.data.activityEndTime ]

            // 步骤二详情
            for(let key in this.ruleForm2) {
              this.ruleForm2[key] = res.data[key] || ''
            }

            // 步骤三详情
            for(let key in this.ruleForm3) {
              this.ruleForm3[key] = res.data[key] || ''
            }
            this.ruleForm3.PrizeDate = [ res.data.cashPrizeBeginTime,  res.data.cashPrizeEndTime ]
            // 兑奖地址的处理
            if (!this.ruleForm3.addressList.length) {
              this.ruleForm3.addressList = [ { list: "" } ]
            } else {
              for(let i =0;i< this.ruleForm3.addressList.length;i++){ 
                this.ruleForm3.addressList[i] = { list: this.ruleForm3.addressList[i] }
              }
            }
            // 步骤四详情

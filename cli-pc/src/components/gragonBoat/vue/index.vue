...
<div class="hd-common">
    <el-breadcrumb separator="/" class="gt-crumbs">
      ...
      <el-breadcrumb-item>端午赛龙舟</el-breadcrumb-item>  
    </el-breadcrumb>
...
<script>
import { getDragonboatList,getDragonboatCount,getMobileUrl,getShortUrl,delDragonboat}from './../api/api'
  export default{
    data() {
      return { 
        keyWord:"",
    ...
    methods: { 
      ...
      getdata(){
        ...
        getDragonboatList(params).then(data=>{
          if (data.code == 100) {
            this.tableData=data
      ...
      delBtn(val) {
            ...
            delDragonboat({id:val}).then(data => {  
                if (data.code == 100) { 
      ...
      //中奖记录
      record(val){
         this.$router.push({path: '/gragonBoat/prizeRecord', query: {id: val}});
      },
      //核销授权
      impower(val){
         this.$router.push({path: '/gragonBoat/cancelOut', query: {id: val}});
      },
      addActive(){

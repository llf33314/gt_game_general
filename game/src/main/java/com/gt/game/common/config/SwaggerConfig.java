...
public class SwaggerConfig {
    // 扫描包
	private static final String  BASEPACKAGE         = "com.gt.game.core.controller";
    // 标题
    private static final String  TITLE               = "Apply API";
    // 描述
    private static final String  DESC                = "多粉项目游戏类API";
    // 版本
    ...
    public Docket createRestApi() {
	    return new Docket( DocumentationType.SWAGGER_2 ).apiInfo( apiInfo() ).tags( new Tag( "game", "多粉项目游戏类API" ), getTags() ).select()
			.apis( RequestHandlerSelectors.basePackage( BASEPACKAGE ) ).paths( PathSelectors.any() ).build();
    }

	private Tag[] getTags() {
        Tag[] tags = { new Tag( "Game", "多粉项目-游戏类相关接口" ) };
        return tags;
    }

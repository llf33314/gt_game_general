...
public class DelFileUtils {
	...
	public static boolean deleteDirectory(String dir) {
		...
		for (int i = 0; i < files.length; i++) {
			// 删除子文件
			if (files[i].isFile()) {
				flag = DelFileUtils.deleteFile(files[i].getAbsolutePath());
				if (!flag)
					break;
			}
			// 删除子目录
			else if (files[i].isDirectory()) {
				flag = DelFileUtils.deleteDirectory(files[i].getAbsolutePath());
				if (!flag)
					break;
			}
		}
	...
	public static void main(String[] args) {
		...
		//DelFileUtils.deleteDirectory(dir);
		DelFileUtils.delete(dir);
		// System.out.println();
		// // 删除文件
		// dir = "c:/test/test0";

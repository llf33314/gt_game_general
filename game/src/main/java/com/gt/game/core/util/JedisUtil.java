...
public class JedisUtil {
	...
	@Value("${apply.redis.host}")
	private  String host;

	@Value("${apply.redis.port}")
	private  String port;

	@Value("${apply.redis.password}")
	...
	public  JedisPool getPool() {
		if (pool == null) {
			...
			if(CommonUtil.isNotEmpty(password)){
				pool = new JedisPool(config,host,CommonUtil.toInteger(port),60000,password);
			}else{				
				pool = new JedisPool(config,host,CommonUtil.toInteger(port),60000);
			}
		}
		return pool;
	}
